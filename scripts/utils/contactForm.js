...
const closeModalElt = document.getElementById("closeForm");
closeModalElt.addEventListener("keydown", function (event) {
  if (event.key === "Enter") {
    closeModal();
  }
});
...
document.addEventListener("keydown", function (event) {
  if (event.key === "Escape") {
    closeModal(); // Ferme la boîte de dialogue si Échap est pressé
  }
});
